...
//        textField.setPromptText("enter a name for your game state: ");

//        String serialisation = model.serialise(textField.getText());

        TextInputDialog textInput = new TextInputDialog();
        textInput.setTitle("Serialisation");
        textInput.setHeaderText("Enter your game name here:");
        textInput.showAndWait();

        boolean check = model.serialise(textInput.getResult(), false);
        if (check == false){
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("name exist");
            alert.setHeaderText("Save name exists, are you sure you want to overwrite?");
            Optional<ButtonType> result = alert.showAndWait();
            if (result.isPresent() && result.get() == ButtonType.OK){
                model.serialise(textInput.getResult(), true);
            }
        }
    }
    ...
    private void deserialiseAction() {
        ...
        TextInputDialog textInput = new TextInputDialog("");
        textInput.setTitle("Serialisation");
        textInput.setHeaderText("Enter your game name string:");
